...
//--------------------------Render GOOOGLE ------------------------------------//
export const renderGoogle = (google, googleClickMe) => {
    ...
    if(subsense){
        if(subsense.examples){
            return subsense.examples.map(example => {
                ...
                return <p id="subExamples" className="ml-5 font-italic">"{example.text}"</p>
            })
        } else {
            return null
        }
...
export const test = (ref) => {
    return <button onClick={(e) => ref.deleteRow(e)}>CLICK ME</button>
}
